...
class StocktakeForm extends \fw\view\form\StdCRUDForm {
    ...
    public function formscript() {
        ...
        $script .= "        var hasEntry = false, hasNegative = false;\n";
        $script .= "        jQuery('.vols-stocktake-qty').each(function() {\n";
        $script .= "            var v = jQuery(this).val();\n";
        $script .= "            if (v !== '') {\n";
        $script .= "                hasEntry = true;\n";
        $script .= "                if (parseFloat(v) < 0) { hasNegative = true; }\n";
        $script .= "            }\n";
        $script .= "        });\n";
        $script .= "        if (hasNegative) {\n";
        $script .= "            jQuery.volsdialog('OKMSG', 'Stock counts cannot be negative.', undefined, undefined, 'Validation');\n";
        $script .= "            return false;\n";
        $script .= "        }\n";
        $script .= "        if (!hasEntry) {\n";
